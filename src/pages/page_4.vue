<template>
	<div class="fx-box">
        ...
        <div class="fx-echarts">
            <div class="fxbox5-txt">在{{studentsAbilityCount}}个维度上，总共为学生提升能力<span class="improvement">{{studentsAbilityTotal}}</span>点,人均提升<span class="Promote">{{studentsAbilityAvg}}</span>点</div>	 
            <div></div>
            <div id="chart_example" style="height: 100%;width:100%"></div>
        </div>
        <div class="fx-table" >
            <div v-if="!array.length&&!good_comment_rate.length&&!supply_join_rate.length" class="co-availability">没有课程记录，无法展示</div>
                <table>
                    <tr>
                        <td>
                            <div>课程供给</div>
                            <div>
                                <span class="iconfont icon-ketang"></span><span class="kaike">开课</span><span class="iconfont icon-rengong-copy"></span><span class="renci">人次</span> 
                            </div>
                        </td>
                    ...
                    <tr>
                        ...
                        <td v-for="(item, index) in summary" :key="index">{{parseFloat(item.good_comment_rate).toFixed(2) + "%"}}</td>
                    </tr>
                    <tr>
                        <td>课程<p>供参比</p></td>
                        <!-- <td v-for="item in supply_join_rate" :key="item.id">{{item}}</td> -->
                        <td v-for="(item, index) in summary" :key="index">{{parseFloat(item.supply_join_rate).toFixed(2) + "%"}}</td>
                    </tr>
            </table>
        </div>
	</div>
           ...
           mounted() { 
            ...
            methods: {
                ...
                getAblitycourse(token,Start,End,Unit,Grade){
                    var _this=this;
                      axios.get(_this.url+'/api/Plat/ability/summary?access_token='+token+'&Start='+Start+'&End='+End+'&Unit='+Unit+'&Grade='+Grade)
                            .then(function (response) {
                             if(response.data.code!==0){
                                sessionStorage.removeItem("token");//清除失效的token
                                window.location.href=" http://class-admin.univteam.com/"+url22+"/account/login?back=statistics";
