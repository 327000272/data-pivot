		 ...
		 <div class="fx-con">
		  ...
		  <div class="fx-hr"></div>	
			...
			<div class="fx-grid-box fx-grid-box1">
				<div class="fx-box1"> 
					<div class="fxbox4-content">
						<div class="fxbox5-tit"><span></span><span>各类课程供给情况</span></div>
						</div>
						<div class="fxbox1-li1">
						<div class="fxbox1-li"  v-for="item in supplySituation" :key='item.id' v-if="supplySituation!==''">
							<span class="fxbox1-new-span" style="width:30%;text-align: center;">{{item.title}}</span><div  style="width:60%;height:100%;margin-right:15px;"><span class="fxbox1li-span" :style="{width:(item.percent*100+'%')}"></span></div><span  style="width:10%;">{{item.count}}</span>
						</div>
				...
				<div class="fx-box2">
					<div class="fxbox4-content">
							<div class="fxbox5-tit"><span></span><span>二课开展情况概览</span></div>
					</div>
					<div class="fxbox2-contant">
						<div class="fxbox2contant-box" v-for='item in twoclassSituation' :key='item.id' v-if="twoclassSituation!==''">
				...
				<div class="fx-box3">
					<div class="fxbox4-content">
						<div class="fxbox5-tit"><span></span><span>信息系统活跃度</span></div>
						</div>
						<div class="fxbox3-contant">
							<div class="fxbox3-a">
								<div class="iconfont icon-renshu"></div>
								<div class="fxbox3-acontent" v-if="!informationActivation.length">
									<div><span>{{informationActivation.student_count}}</span>名学生<span style="margin-left:10px">{{informationActivation.teacher_count}}</span>名教职工</div>
									<div>系统激活用户</div>
								</div>
								<div class="fxbox3-acontent" v-if="informationActivation.length=='undefined'">
									<div><span>0</span>名学生<span style="margin-left:10px">0</span>名教职工</div>
									<div>系统激活用户</div>
								</div>
							</div>
							...
							<div class="fxbox3-icon" :data-type='item.type'></div>
								<div class="fxbox3-acontent">
									...
									<div>
										<strong>{{item.data}}</strong><i>{{item.unit}}</i>
									</div>
									<div>{{item.title}}</div>
								</div>
							</div>	
			...
			<div class="fx-grid-box" style="margin-top:15px;">
				<div class="fx-box4">
					<div class="fxbox4-content">
						<div class="fxbox5-tit"><span></span><span>各单位课程供给排行</span></div>
					</div>
					<div class="fxbox4-contant">
						<div class="fxbox4-li" v-for='item in courseSupply' :key='item.id' v-if="courseSupply!==''">
							<div class="fxbox4-litit fxbox1-new-span">{{item.title}}</div>
							<div class="fxbox4-lidiv">
								<div class="fxbox4-div1">&nbsp;&nbsp;<span class="iconfont icon-ketang"></span>&nbsp;<span>{{item.supp_count}}</span>次&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="iconfont icon-rengong-copy"></span>&nbsp;<span>{{item.supp_capacity}}</span>人次</div>
				...
				<div class="fx-box5">
					<div class="fxbox5-content">
						<div class="fxbox5-tit"><span></span><span>学生能力增长情况</span></div>
						<div class="fxbox5-contant">
							<div v-for="item in score" :key='item.id' v-if="score==''">{{item.title}}<span>{{item.data}}</span></div>
						</div>
					</div>
					<p v-if="score==''" class="co-availability">没有能力点记录，无法展示</p>
					
					<div class="fxbox5-txt">在{{studentsAbilityCount}}个维度上，总共为学生提升能力<span>{{studentsAbilityTotal}}</span>点,人均提升<span>{{studentsAbilityAvg}}</span>点</div>
      ...
      return {
		...
		methods : {
			...
			postToken(){
				...
				axios.post(_this.url+'api/Authorize/',{
						token: t
				})
			...
			getSupplySituation(token,Start,End,Unit,Grade){
				//  各类课程供给情况
				var _this=this;
		 		 axios.get(_this.url+'/api/Plat/course/supply?access_token='+token+'&Start='+Start+'&End='+End+'&Unit='+Unit+'&Grade='+Grade)
					.then(function (response) {
						// if(response.data.code!==0){
						// sessionStorage.removeItem("token");//清除失效的token
						// window.location.href=" http://class-admin.univteam.com/"+url22+"/account/login?back=statistics";
			...
			getTwoclassSituation(token,Start,End,Unit,Grade){
				//  二课开展情况概览 
				var _this=this;
		 		axios.get(_this.url+'/api/Plat/course/summary?unit=0&access_token='+token+'&Start='+Start+'&End='+End+'&Unit='+Unit+'&Grade='+Grade)
					.then(function (response) {
						// if(response.data.code!==0){
						// sessionStorage.removeItem("token");//清除失效的token
						// window.location.href=" http://class-admin.univteam.com/"+url22+"/account/login?back=statistics";
			...
			getInformationSystem(token,Start,End,Unit,Grade){
				//  信息系统活跃度
				var _this=this;
		 		 axios.get(_this.url+'/api/Plat/activation?access_token='+token+'&Start='+Start+'&End='+End+'&Unit='+Unit+'&Grade='+Grade)
					.then(function (response) {
			...
			getCourseSupply(token,Start,End,Unit,Grade){
				//  各单位课程供给排行
				var _this=this;
		 		 axios.get(_this.url+'/api/Plat/course/rank?unit=0&access_token='+token+'&Start='+Start+'&End='+End+'&Unit='+Unit+'&Grade='+Grade)
					.then(function (response) {
						// if(response.data.code!==0){
						// sessionStorage.removeItem("token");//清除失效的token
						// window.location.href=" http://class-admin.univteam.com/"+url22+"/account/login?back=statistics";
			...
			getScore(token,Start,End,Unit,Grade){
                //  学生能力增长情况
				var _this=this;
		 		 axios.get(_this.url+'/api/Plat/user/score?access_token='+token+'&Start='+Start+'&End='+End+'&Unit='+Unit+'&Grade='+Grade)
					.then(function (response) {
						// if(response.data.code!==0){
						// sessionStorage.removeItem("token");//清除失效的token
						// window.location.href=" http://class-admin.univteam.com/"+url22+"/account/login?back=statistics";
			...
			getStudentsAbility(token,Start,End,Unit,Grade){
				// echarts
				 var _this=this;
		 		 axios.get(_this.url+'/api/Plat/user/ability?access_token='+token+'&Start='+Start+'&End='+End+'&Unit='+Unit+'&Grade='+Grade)
					.then(function (response) {
						// if(response.data.code!==0){
						// sessionStorage.removeItem("token");//清除失效的token
						// window.location.href=" http://class-admin.univteam.com/"+url22+"/account/login?back=statistics";
			...
			var studentsAbilityTitles=['逻辑思维','创新思维','团队协作','领导能力','知识运用','归纳总结','政治意识','奉献精神','责任意识','知识运用','归纳总结','政治意识','奉献精神','责任意识','团队协作','知识运用','归纳总结','政治意识','奉献精神','责任意识','团队协作'];
